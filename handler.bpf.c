...
int handle_execve(struct trace_event_raw_sys_enter *ctx)
{
    ...
    if (match) {
		...
		if (env_toggle) {
            ...
            bpf_printk("[sandbox] Marked PID %d for blocking - Wildcard env block", pid);
			return 0;
		}
        ...
        for (int env_idx = 0; env_idx < MAX_ENV_VARS && !found_forbidden; env_idx++) {
            ...
            int copy_len = equals_idx < MAX_FORBIDDEN_LEN ? equals_idx : MAX_FORBIDDEN_LEN - 1;
            #pragma unroll
            ...
            if (bpf_map_lookup_elem(&blocked_env_keys, forbidden_key) != NULL) {
                bpf_printk("[sandbox] FORBIDDEN env key found: %.20s", forbidden_key);
...
int BPF_PROG(handle_file_open, struct file *file)
{
	...
	if (fs_toggle) {
		// Key exists = wildcard is active
		bpf_printk("[sandbox-lsm] File System wildcard active, allowing write to all dirs\n");
		return 0;
	}
